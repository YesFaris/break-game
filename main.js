...
document.addEventListener("DOMContentLoaded", function () {
	...
	returnToLevelSelectButton = document.getElementById(
...
function showLevelCompletePopup() {
	if (levelCompletePopup) {
		levelCompleteMessage.textContent = `NIVEAU ${
			currentLevelIndex + 1
		} TERMINÉ !`;
...
function initGame() {
	...
	const originalTotalBricksWidth =
		currentLevel.brick.columnCount * currentLevel.brick.width +
		(currentLevel.brick.columnCount - 1) * currentLevel.brick.padding;
	brickInfo.offsetLeft =
		((ORIGINAL_CANVAS_WIDTH - originalTotalBricksWidth) / 2) * scaleX;
...
function checkIfMobile() {
	...
	if (mobileControlsDiv) {
		mobileControlsDiv.style.display =
			isMobile && gameStarted && !gameOver && !gameWon && !gamePaused
				? "flex"
				: "none";
	}
	if (touchControlsContainer) {
		touchControlsContainer.style.display =
			isMobile && gameStarted && !gameOver && !gameWon && !gamePaused
				? "flex"
				: "none";
	}
		...
		for (let r = 0; r < brickInfo.rowCount; r++) {
			...
			if (b.status === 1) {
				...
				if (b.hitsLeft < brickInfo.hitsToBreak) {
					...
					currentBrickColor =
						"#" + darkerColorValue.toString(16).padStart(6, "0");
				}
				...
				ctx.fillRect(
					brickX + brickInfo.width - 2 * scaleX,
					brickY,
					2 * scaleX,
					brickInfo.height,
				);
				...
				ctx.fillRect(
					brickX,
					brickY + brickInfo.height - 2 * scaleY,
					brickInfo.width,
					2 * scaleY,
				);
...
function drawScore() {
	...
	ctx.fillText(
		...
		(ORIGINAL_CANVAS_WIDTH - 150) * scaleX,
		30 * scaleY,
	);
	ctx.textAlign = "start";
	return score;
...
function drawAll() {
	...
	const mobileControlsDiv = document.querySelector(
	...
	if (gamePaused) {
		...
		else if (mobileControlsDiv && !isMobile)
			mobileControlsDiv.style.display = "none"; // Cache sur desktop si pas mobile
		if (returnToLevelSelectButton)
			returnToLevelSelectButton.style.display = "none"; // Toujours caché sur desktop en pause
		// S'assurer que les pop-ups de fin de niveau/jeu sont cachés
		if (gameOverPopup) gameOverPopup.style.display = "none";
		if (levelCompletePopup) levelCompletePopup.style.display = "none";
		if (gameCompletePopup) gameCompletePopup.style.display = "none";
	} else if (gameOver) {
		// Si Game Over, afficher le pop-up
		showGameOverPopup(); // Appel inconditionnel, le pop-up gérera sa visibilité
...
function drawGameOverScreen() {
	...
	ctx.fillText(
		"GAME OVER",
		canvasDom.width / 2,
		(ORIGINAL_CANVAS_HEIGHT / 2 - 20) * scaleY,
	);
...
function detectCollisions() {
	...
	if (
		ball.x + ball.dx > canvasWidthScaled - ball.radius ||
		...
		for (let r = 0; r < brickInfo.rowCount; r++) {
			...
			if (b.status === 1) {
				...
				if (
					...
					if (
						prevBallY + ball.radius <= brickTop ||
						prevBallY - ball.radius >= brickBottom
					) {
						ball.dy = -ball.dy;
					} else {
						ball.dx = -ball.dx;
					...
					if (remainingBricks === 0) {
						gameWon = true;
						// drawAll sera appelé dans la prochaine frame et gérera le pop-up
					}
				}
			}
		}
